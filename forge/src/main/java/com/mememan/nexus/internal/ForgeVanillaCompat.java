...
public final class ForgeVanillaCompat {
    ...
    public static void registerVanillaIntegration() {
        ...
        TagWrapper.getCachedTWEntries().stream().filter(curTw -> curTw.getParentTag().get().isFor(Registries.BLOCK) && curTw.getFlammabilitySettings() != null).forEach(curTw -> {
            ...
            for (Holder<Block> blockEntryHolder : BuiltInRegistries.BLOCK.getTagOrEmpty((TagKey<Block>) curTw.getParentTag().get())) { // Forge doesn't provide an alt (Native for() loops FTW)
