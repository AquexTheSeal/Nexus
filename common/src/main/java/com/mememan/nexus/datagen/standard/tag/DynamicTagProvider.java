...
public abstract class DynamicTagProvider<T> extends TagsProvider<T> implements ModDataProvider {
    protected final String modId;
    protected final boolean validateAllEntries;
    protected final DuplicateDataPolicy dupeStrat;
    protected final Object2ObjectOpenHashMap<ResourceKey<T>, ObjectArrayList<TagKey<T>>> trackedTaggedObjects = new Object2ObjectOpenHashMap<>();

    public DynamicTagProvider(PackOutput output, ResourceKey<? extends Registry<T>> targetTagObjectRegistry, CompletableFuture<HolderLookup.Provider> objectLookupProvider, String modId, boolean validateAllEntries, DuplicateDataPolicy dupeStrat) {
        super(output, targetTagObjectRegistry, objectLookupProvider);
    ...
    public DynamicTagProvider(PackOutput output, ResourceKey<? extends Registry<T>> targetTagObjectRegistry, CompletableFuture<HolderLookup.Provider> objectLookupProvider, CompletableFuture<TagLookup<T>> tagLookup, String modId, boolean validateAllEntries, DuplicateDataPolicy dupeStrat) {
        super(output, targetTagObjectRegistry, objectLookupProvider, tagLookup);
    ...
    @Nullable
    protected String getObjectName(ResourceKey<T> targetObjKey) {
        return targetObjKey.location().toString();
    }
    ...
    protected boolean validateDupeObjectTag(ResourceKey<T> targetObjectKey, TagKey<T> targetTagKey) {
        ObjectArrayList<TagKey<T>> targetObjTags = trackedTaggedObjects.computeIfAbsent(targetObjectKey, oK -> new ObjectArrayList<>());
        ...
        if (targetObjTags.stream().map(TagKey::location).anyMatch(tagLoc::equals)) {
            switch (getDuplicateDataPolicy()) {
                case CRASH -> throw new IllegalStateException(String.format("Attempted to tag %s %s with duplicate tag key %s from mod of ID %s, specified DuplicateDataPolicy is CRASH.", potentialTypeName, getObjectName(targetObjectKey), tagLoc, getModId()));
                case EXCLUDE_WARN -> {
                    NexusConstants.LOGGER.warn("Attempted to tag {} {} with duplicate tag key {} from mod of ID {}, specified DuplicateDataPolicy is EXCLUDE_WARN. Skipping...", potentialTypeName, getObjectName(targetObjectKey), tagLoc, getModId());
                    return false;
                }
                case EXCLUDE_SILENT -> {
                    return false;
                ...
                case OVERRIDE_WARN -> {
                    NexusConstants.LOGGER.warn("Overriding duplicate {} tag {} for {} {} from mod of ID {}, specified DuplicateDataPolicy is OVERRIDE_WARN.", potentialTypeName, tagLoc, potentialTypeName, getObjectName(targetObjectKey), getModId());
                    return true;
                }
