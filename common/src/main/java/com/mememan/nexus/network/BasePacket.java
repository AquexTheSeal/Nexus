...
public record BasePacket<MSGT>(ResourceLocation packetId, Class<MSGT> packetClass, BiConsumer<MSGT, FriendlyByteBuf> packetEncoder, Function<FriendlyByteBuf, MSGT> packetDecoder, Function<MSGT, PacketContext> packetHandler, NetworkSide targetSide) {
    ...
    public BasePacket(String modId, Class<MSGT> packetClass, BiConsumer<MSGT, FriendlyByteBuf> packetEncoder, Function<FriendlyByteBuf, MSGT> packetDecoder, Function<MSGT, PacketContext> packetHandler, NetworkSide targetSide) {
        this(new ResourceLocation(modId, packetClass.getSimpleName().replaceAll("([a-z])([A-Z])", "$1_$2").toLowerCase(Locale.ROOT).replace("_packet", "")), packetClass, packetEncoder, packetDecoder, packetHandler, targetSide);
    }
}
